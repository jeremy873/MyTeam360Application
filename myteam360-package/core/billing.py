...
def create_checkout_session(db, user_id, user_email, plan="monthly"):
    """Create a Stripe Checkout session and return the URL."""
    if not _ensure_stripe():
        return {"error": "Stripe not configured"}

    price_id = os.getenv("STRIPE_PRICE_MONTHLY") if plan == "monthly" else os.getenv("STRIPE_PRICE_ANNUAL")
    if not price_id:
        return {"error": f"Stripe price ID not set for {plan} plan"}
...
def register_billing_routes(app, get_db_func):
    ...
    def billing_checkout():
        ...
        with get_db_func() as db:
            _init_billing_db(db)
            user = g.user
            result = create_checkout_session(db, g.user_id, user.get("email", ""), plan)
            if "error" in result:
