...
class SecurityManager:
    ...
    def _check_auth(self, ip: str):
        ...
        if session.get("user_id"):
            ...
            if user and user["is_active"]:
                g.user = user
                g.user_id = user["id"]
                g.user_name = user.get("display_name", "")
                g.first_name = (user.get("display_name", "") or "").split()[0] if user.get("display_name") else ""
                return None
            session.clear()
    ...
    def _validate_token(self, token: str, ip: str):
        ...
        g.user_id = row["uid"]
        g.user_name = row.get("display_name", "")
        g.first_name = (row.get("display_name", "") or "").split()[0] if row.get("display_name") else ""
        return None
...
.logo{width:48px;height:48px;background:linear-gradient(135deg,#a459f2,#c084fc);border-radius:12px;display:inline-flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;color:#fff;margin-bottom:16px}
...
input:focus{border-color:#a459f2}
button{width:100%;background:#a459f2;border:none;border-radius:8px;padding:12px;color:#fff;font-size:14px;font-weight:600;cursor:pointer;margin-top:8px;transition:.15s}
