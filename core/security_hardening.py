...
# ══════════════════════════════════════════════════════════════
# 1. ENCRYPTION AT REST — AES-256-GCM field encryption
# ══════════════════════════════════════════════════════════════
...
class FieldEncryptor:
    ...
    derived from an environment variable or auto-generated and stored locally.

    AES-256-GCM provides:
      - 256-bit key (vs Fernet's 128-bit)
      - Authenticated encryption (integrity + confidentiality)
      - 96-bit nonce per encryption (unique per message)
      - No padding needed (stream cipher mode)

    Backward compatible: reads old Fernet "enc:" data, writes new "enc2:" data.
    """

    def __init__(self):
        self._key = None       # 32-byte AES-256 key
        self._fernet = None    # Legacy Fernet for reading old data
        self._init_key()
    ...
    def _init_key(self):
        ...
        if master:
            # If it's a Fernet key (44 chars base64), derive AES-256 from it + keep Fernet for compat
            try:
                raw = base64.urlsafe_b64decode(master.encode() if isinstance(master, str) else master)
                if len(raw) == 32:
                    # Fernet key = 16 bytes signing + 16 bytes encryption
                    # Use full 32 bytes as our AES-256 key
                    self._key = raw
                    self._init_legacy_fernet(master)
                else:
                    self._key = raw[:32].ljust(32, b'\0')
            except Exception:
                # Plain text key — hash it to get 256 bits
                import hashlib
                self._key = hashlib.sha256(master.encode()).digest()
            return
        ...
        if os.path.exists(key_path):
            ...
            try:
                ...
                if len(raw) == 32:
                    self._key = raw
                    self._init_legacy_fernet(stored.decode())
                else:
                    self._key = raw[:32].ljust(32, b'\0')
            except Exception:
                import hashlib
                self._key = hashlib.sha256(stored).digest()
            return
    ...
    def _init_legacy_fernet(self, key_str):
        """Initialize Fernet for reading legacy enc: data."""
        try:
    ...
    def encrypt(self, plaintext):
        """Encrypt a string with AES-256-GCM. Returns 'enc2:' prefixed token."""
        if not plaintext or not self._key:
            return plaintext
        ...
        # Store as: nonce (12 bytes) + ciphertext (includes 16-byte auth tag)
        token = base64.urlsafe_b64encode(nonce + ciphertext).decode()
        return "enc2:" + token

    def decrypt(self, ciphertext):
        """Decrypt 'enc2:' (AES-256-GCM) or legacy 'enc:' (Fernet) data."""
        if not ciphertext:
            return ciphertext
        ...
        if ciphertext.startswith("enc2:"):
            ...
            try:
                ...
                raw = base64.urlsafe_b64decode(ciphertext[5:])
                nonce = raw[:12]
                ct = raw[12:]
                aesgcm = AESGCM(self._key)
        ...
        # Legacy Fernet format — backward compatible
        if ciphertext.startswith("enc:"):
            if self._fernet:
                try:
                    ...
                    return None
            logger.error("Legacy enc: data found but no Fernet key available")
            return None
    ...
    def is_encrypted(self, value):
        return isinstance(value, str) and (value.startswith("enc2:") or value.startswith("enc:"))

    def rotate_key(self, new_key=None):
        """Re-encrypt all sensitive fields with a new AES-256 key."""
        from cryptography.hazmat.primitives.ciphers.aead import AESGCM
        import base64
        ...
        if isinstance(new_key, str):
            import hashlib
            new_key = hashlib.sha256(new_key.encode()).digest()
        ...
        with get_db() as db:
            ...
            for row in rows:
                ...
                for field in ["api_key", "oauth_client_secret", "oauth_access_token", "oauth_refresh_token"]:
                    val = row[field]
                    if val and (val.startswith("enc2:") or val.startswith("enc:")):
                        plain = self.decrypt(val)
                        if plain:
                            # Re-encrypt with new key using AES-256-GCM
                            nonce = os.urandom(12)
                            aesgcm = AESGCM(new_key)
                            ct = aesgcm.encrypt(nonce, plain.encode(), None)
                            new_val = "enc2:" + base64.urlsafe_b64encode(nonce + ct).decode()
        ...
        # Save new key
        self._key = new_key
        self._fernet = None  # No more legacy
        key_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", ".encryption_key")
        with open(key_path, "wb") as f:
...
# ══════════════════════════════════════════════════════════════
# 4b. MFA ENFORCEMENT + TRUSTED DEVICES + SENSITIVE OPS GUARD
# ══════════════════════════════════════════════════════════════

class MFAEnforcement:
    """Admin can require MFA for all users, specific roles, or specific actions.

    Enforcement levels:
      - off: MFA is optional
      - recommended: Users see a banner nudging them to enable MFA
      - required_admins: Admin/owner roles must have MFA
      - required_all: All users must have MFA (enforced at login)
    """

    LEVELS = ["off", "recommended", "required_admins", "required_all"]

    def get_policy(self) -> dict:
        with get_db() as db:
            row = db.execute(
                "SELECT value FROM workspace_settings WHERE key='mfa_enforcement'"
            ).fetchone()
        level = dict(row)["value"] if row else "off"
        return {"level": level, "options": self.LEVELS}

    def set_policy(self, level: str, admin_id: str = "") -> dict:
        if level not in self.LEVELS:
            return {"error": f"Invalid level. Must be one of: {self.LEVELS}"}
        with get_db() as db:
            db.execute(
                "INSERT OR REPLACE INTO workspace_settings (key, value) VALUES ('mfa_enforcement', ?)",
                (level,))
        logger.info(f"MFA enforcement set to '{level}' by {admin_id}")
        return {"level": level}

    def check_enforcement(self, user_id: str, user_role: str,
                          mfa_enabled: bool) -> dict:
        """Check if this user is compliant with the MFA policy."""
        policy = self.get_policy()
        level = policy["level"]

        if level == "off" or level == "recommended":
            return {"compliant": True, "required": False, "nudge": level == "recommended" and not mfa_enabled}

        if level == "required_admins" and user_role in ("owner", "admin"):
            if not mfa_enabled:
                return {"compliant": False, "required": True,
                        "message": "Admin accounts require MFA. Please set up MFA to continue."}
            return {"compliant": True, "required": True}

        if level == "required_all":
            if not mfa_enabled:
                return {"compliant": False, "required": True,
...
class TrustedDeviceManager:
    """Remember trusted devices so users don't enter MFA every login.

    After successful MFA verification, the user can "trust this device"
    for 30 days. Creates a signed cookie that bypasses MFA on next login.
    """

    TRUST_DURATION_DAYS = 30

    def create_device_token(self, user_id: str, ip: str, user_agent: str) -> str:
        """Create a signed device trust token."""
        fingerprint = hashlib.sha256(f"{user_id}:{ip}:{user_agent}".encode()).hexdigest()[:16]
        expires = (datetime.now() + timedelta(days=self.TRUST_DURATION_DAYS)).isoformat()
        token_data = f"{user_id}|{fingerprint}|{expires}"
        signature = hashlib.sha256(f"{token_data}|{os.getenv('SECRET_KEY', 'mt360')}".encode()).hexdigest()[:16]
        return f"{token_data}|{signature}"

    def validate_device_token(self, token: str, user_id: str,
                               ip: str, user_agent: str) -> bool:
        """Validate a device trust token."""
        if not token:
            return False
        try:
            parts = token.split("|")
            if len(parts) != 4:
                return False
            t_user, t_fp, t_expires, t_sig = parts

            if t_user != user_id:
                return False

            # Check expiry
            expires_dt = datetime.fromisoformat(t_expires)
            if datetime.now() > expires_dt:
                return False

            # Check fingerprint
            expected_fp = hashlib.sha256(f"{user_id}:{ip}:{user_agent}".encode()).hexdigest()[:16]
            if t_fp != expected_fp:
                return False

            # Verify signature
            token_data = f"{t_user}|{t_fp}|{t_expires}"
            expected_sig = hashlib.sha256(f"{token_data}|{os.getenv('SECRET_KEY', 'mt360')}".encode()).hexdigest()[:16]
            if t_sig != expected_sig:
                return False
    ...
    def store_trust(self, user_id: str, device_token: str):
        ...
        with get_db() as db:
            try:
                ...
                """, (user_id, hashlib.sha256(device_token.encode()).hexdigest(),
                      (datetime.now() + timedelta(days=self.TRUST_DURATION_DAYS)).isoformat()))
            except:
                pass

    def is_trusted(self, user_id: str, device_token: str) -> bool:
        ...
        with get_db() as db:
            try:
                row = db.execute(
                    "SELECT expires_at FROM trusted_devices WHERE user_id=? AND device_token=?",
                    (user_id, token_hash)).fetchone()
            except:
                return False
        if not row:
            return False
        try:
            return datetime.now() < datetime.fromisoformat(dict(row)["expires_at"])
        except:
            return False
...
class SensitiveOperationGuard:
    ...
    Even if the user is logged in, some operations need a fresh MFA check:
    - Changing password
    - Creating/viewing API keys
    - GDPR data erasure
    - Disabling MFA
    - Changing email
    - Admin operations (user management, compliance review)
    - Exporting all data

    The user provides their MFA code with the request.
    A verified code grants a 5-minute window for the operation.
    """

    SENSITIVE_PATHS = [
        "/api/me/password",
        "/api/security/gdpr/erase",
        "/api/security/mfa/disable",
        "/api/security/encryption/rotate",
        "/api/data-export/full",
    ]

    WINDOW_SECONDS = 300  # 5 minutes after MFA verification

    def __init__(self):
        self._verified = {}  # user_id → timestamp of last MFA verification

    def is_sensitive(self, path: str) -> bool:
        """Check if a request path requires MFA re-verification."""
        return any(path.startswith(p) for p in self.SENSITIVE_PATHS)

    def record_verification(self, user_id: str):
        """Record that user just passed MFA for sensitive ops."""
        self._verified[user_id] = time.time()

    def is_verified(self, user_id: str) -> bool:
        """Check if user has a recent MFA verification."""
        ts = self._verified.get(user_id)
        if not ts:
            return False
        return (time.time() - ts) < self.WINDOW_SECONDS

    def check_or_require(self, user_id: str, path: str, mfa_enabled: bool) -> dict:
        """Check if this request needs MFA and if user has verified recently."""
        if not self.is_sensitive(path):
            return {"allowed": True}

        if not mfa_enabled:
            # MFA not enabled — allow but warn
            return {"allowed": True, "warning": "Consider enabling MFA for added security on sensitive operations."}
        ...
        return {
            "allowed": False,
            "mfa_required": True,
            "message": "This operation requires MFA verification. Please provide your current MFA code.",
            "verify_endpoint": "/api/security/mfa/verify-sensitive",
        }


class MFARecovery:
    """Recovery path when user loses phone AND all backup codes.

    Process:
    1. User requests recovery via email
    2. System sends a time-limited recovery link (1 hour)
    3. User must verify identity (email + security question or admin approval)
    4. MFA is temporarily disabled for one login
    5. User must re-enable MFA immediately after login
    6. All trusted devices are revoked
    """

    def initiate_recovery(self, email: str) -> dict:
        """Start the MFA recovery process."""
        with get_db() as db:
            user = db.execute("SELECT id, display_name FROM users WHERE email=?",
                             (email,)).fetchone()
        if not user:
            # Don't reveal if email exists — timing-safe response
            return {"initiated": True,
                    "message": "If this email is associated with an account, a recovery link has been sent."}
        ...
        with get_db() as db:
            try:
                db.execute("""
                    INSERT OR REPLACE INTO mfa_recovery
                        (user_id, recovery_token, expires_at, used)
                    VALUES (?,?,?,0)
                """, (user_id, hashlib.sha256(recovery_token.encode()).hexdigest(), expires))
            except:
                pass

        # In production: send email with recovery link
        logger.info(f"MFA recovery initiated for {user_id}")

        return {
            "initiated": True,
            "message": "If this email is associated with an account, a recovery link has been sent.",
            # In production: don't return the token — send via email
            "_dev_recovery_token": recovery_token,
        }

    def complete_recovery(self, recovery_token: str) -> dict:
        """Complete MFA recovery — temporarily disable MFA for one login."""
        token_hash = hashlib.sha256(recovery_token.encode()).hexdigest()

        with get_db() as db:
            row = db.execute(
                "SELECT user_id, expires_at, used FROM mfa_recovery WHERE recovery_token=?",
                (token_hash,)).fetchone()
        ...
        r = dict(row)
        if r.get("used"):
            return {"error": "This recovery token has already been used."}
        ...
        with get_db() as db:
            # Mark token as used
            db.execute("UPDATE mfa_recovery SET used=1 WHERE recovery_token=?", (token_hash,))
            # Temporarily disable MFA
            db.execute("UPDATE mfa_config SET is_enabled=0 WHERE user_id=?", (user_id,))

        # Revoke all trusted devices
        TrustedDeviceManager().revoke_all(user_id)

        logger.warning(f"MFA recovery completed for {user_id} — MFA temporarily disabled")

        return {
            "recovered": True,
            "user_id": user_id,
            "message": "MFA has been temporarily disabled. You must re-enable MFA after your next login.",
        }
